...
{ ///
  assert(img != NULL);
  // Insert your code here!
  Image img_result = ImageCreate(img->height, img->width, img->maxval);

  for (int i = img->width - 1; i >= 0; i--)
  {
    for (int j = 0; j < img->height; j++)
    {
      ImageSetPixel(img_result, j, i, ImageGetPixel(img, img->width - i - 1, j));
    }
  }

  int success = check(img_result != NULL, "ImageRotate failed");
...
{ ///
  assert(img != NULL);
  // Insert your code here!
  Image img_result = ImageCreate(img->width, img->height, img->maxval);

  for (int i = 0; i < img->width; i++)
  {
    for (int j = 0; j < img->height; j++)
    {
      ImageSetPixel(img_result, img->width - i - 1, j, ImageGetPixel(img, i, j));
    }
  }

  int success = check(img_result != NULL, "ImageMirror failed");
...
{ ///
  ...
  Image img_result = ImageCreate(w, h, img->maxval);

  for (int i = 0; i < w; i++)
  {
    for (int j = 0; j < h; j++)
    {
      ImageSetPixel(img_result, i, j, ImageGetPixel(img, x + i, y + j));
    }
  }
